  ...
  if (!result) {
    return (
      <div className="app-body">
        <div className="state-wrap"><p>Tidak ada data transaksi.</p>
    ...
    <div className="app-body">
      <div className="success-wrap">
        ...
        <div className="receipt">
          ...
          <div className="kv"><span className="k">Status</span><span className="v"><span className="chip done"><Check size={11} /> Terkonfirmasi</span></span></div>
          <div className="kv"><span className="k">Ledger</span><span className="v">#{result.ledger}</span></div>
          <div className="kv"><span className="k">Waktu</span><span className="v">~5 detik</span></div>
        </div>
      </div>
      <div className="pad">
        <button className="btn btn-primary" style={{ marginBottom: "var(--space-md)" }} onClick={() => push("track")}>
          Lacak status <ArrowRight size={16} />
        </button>
        <button className="btn btn-ghost" onClick={() => goTab("home")}>
          <HomeIcon size={15} /> Kembali ke beranda
        </button>
      </div>
    </div>
  );
